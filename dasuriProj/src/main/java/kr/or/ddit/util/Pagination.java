...
public class Pagination {
	...
	public Pagination(int total, int currentPage, int size) {
		...
		if(total ==0) {	//전체 게시글의 개수가 0(게시글이 없다면..)
			...
			//17(total) / 3(size 한 화면 보여질 행 수) => 5
			totalPages = total /size;
			if(total % size > 0) {
				totalPages++; //6(totalPages)
			}
			
			//시작 페이지 번호 구하는 공식 : 현재 페이지 / 페이징 크기 * 페이징 크기 + 1;
			startPage = currentPage / 5 * 5 + 1;
			
			//만약에 (사용자 현재 페이지 % 페이징 크기)가 0이면 결과 - 페이징 크기를 해줘야 함
			int modVal = currentPage % 5;
			if(modVal == 0) startPage -= 5;
			
			//종료 페이지 번호 구하는 공식 : 시작 페이지 번호 + (페이징 크기-1)
			endPage = startPage + 4;
			
			//종료 페이지 번호가 전체 페이지보다 크면 종료 페이지 번호를 전체 페이지 번호로 보정해줘야 함
