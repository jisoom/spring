...
public class PdfServiceImpl implements PdfService{
	...
	public String createPdf(List<EmpVO> empVoList) {
		...
		try {
			...
			//어디에 만들어지나 ?
			//writer 객체는 pdf 문서를 작성해주는 작성자. 싱글톤 패턴으로 객체 생성
			...
			//한글폰트를 처리해줘야 함
			//c드라이브의 windows > fonts 폴더의 한글 폰트 사용(malgun.ttf)
			...
			//5개의 셀로 구성된 테이블 객체를 생성함
			PdfPTable pdfPTable = new PdfPTable(5);
			
			//타이틀 처리. 한글 글꼴이므로 위에서 설정한 font 객체를 사용
			...
			//document 내의 table 객체에 cell을 추가해줌
			pdfPTable.addCell(cellRnum);
			pdfPTable.addCell(cellNm);
			pdfPTable.addCell(cellAddr);
			pdfPTable.addCell(cellPne);
			pdfPTable.addCell(cellSal);
			...
			//document(pdf문서 자체) 객체에 직원 목록이 담긴 cell집합(pdfPTable)을 넣어줌
