...
<!-- 			<a class="btn btn-app" onclick="javascript:location.href='/emp/pdfDown'"> -->
			<a class="btn btn-app" id="makePdf">
				<i class="fas fa-save"></i> Pdf 다운로드
			</a>
		</div>
					...
					<table id="example2"
						...
						<thead>
							<tr role="row">
								...
								<th class="sorting" tabindex="0" aria-controls="example2"
									rowspan="1" colspan="1"
									aria-label="CSS grade: activate to sort column ascending">월급</th>
							</tr>
						</thead>
						<tbody>
						<c:forEach var="empVo" items="${list}" varStatus="status"> <!-- (${status.count})  -->
...
<%-- choose문 연습 	
<c:set var="name" value="김은대" />
	<c:choose>
		<c:when test="${name=='이쁜이'}">
			김은대가 아닙니다
		</c:when>
		<c:when test="${name=='김은대'}">
			김은대가 맞습니다.
		</c:when>
		<c:otherwise>
			사람이 아닙니다.
